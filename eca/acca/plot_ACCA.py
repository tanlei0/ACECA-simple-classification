...
def dict2jsonfile(dict_data=None, json_path=""):
    jsObj = json.dumps(dict_data, indent=4)  # indent参数是换行和缩进
...
def get_run_datas_multi(save_path=None):
    ...
    # ca_run_nums = [10, 20, 30, 40, 50, 60, 70, 80]
    rule = 90
    init_state = '0' * 100 + '1' + '0' * 100
    m2_gap = [12, 24, 60]
    m3_gap = [8, 16, 40]
    ...
    for ca_run_num in ca_run_nums:
        pool.apply_async(run_func,
                         (alphas, template_, rule, init_state, ca_run_num,
                          lock))  # 维持执行的进程总数为processes，当一个进程执行完毕后会添加新的进程进去
    pool.close()
    pool.join()
    new_temp = {'m2': {}, 'm3': {}}
    for k in template_:
