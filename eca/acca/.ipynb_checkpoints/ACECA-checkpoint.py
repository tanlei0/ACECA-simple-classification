...
if __name__ == "__main__":
    init_state = getInitState(100, d_ini=0.3)
    # init_state = '0' * 2 + '1' + '0' * 2
    # init_state = '0' * 48 + '1' + '0' * 48
    run_num = 300
    rule = 90
    aceca = ACECA(rule=rule, init_state=init_state, alpha=0.5, run_num=run_num)
    aceca.run(isPrint=True)

    # 绘制图形
    plot_space(space=aceca.space)
    ...
    for step in steps:
        ...
        plot_dense(density=density, title="ACA_run{}_rule{}_cell{}".format(aceca.run_num, aceca.rule, aceca.n_cell))
